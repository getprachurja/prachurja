...
const schemas = {
  plants: z.object({ id: slug, commonName: shortText, botanicalName: shortText, region: shortText, ecologicalRole: shortText, waterRequirement: z.enum(["Low", "Moderate", "High"]), plantSize: shortText, price: z.number().min(0).max(10_000_000), stockStatus: z.enum(["Available", "Low stock", "Contract grow"]), imageUrl: url, description: longText, featured: z.boolean(), active: z.boolean(), sortOrder: z.number().int().min(0).max(10_000) }),
  products: z.object({ id: slug, name: shortText, category: shortText, producer: shortText, location: shortText, price: z.number().min(0).max(10_000_000), badge: z.string().trim().max(120), imageUrl: url, description: longText, featured: z.boolean(), active: z.boolean(), sortOrder: z.number().int().min(0).max(10_000) }),
  blogs: z.object({ id: slug, title: shortText, excerpt: z.string().trim().min(10).max(600), body: z.string().trim().min(20).max(40_000), category: shortText, readTime: shortText, coverImageUrl: url, status: z.enum(["Draft", "Published"]), featured: z.boolean() }),
  projects: z.object({ id: slug, title: shortText, location: shortText, area: shortText, projectType: shortText, status: shortText, speciesCount: z.number().int().min(0).max(100_000), period: shortText, progress: z.number().int().min(0).max(100), summary: z.string().trim().min(10).max(4000), imageUrl: url, featured: z.boolean(), published: z.boolean(), sortOrder: z.number().int().min(0).max(10_000) }),
} as const;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const items = await selectRows(tables[resource.data], {
      order: { column: orderColumn, ascending: resource.data !== "blogs" },
    });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (resource.data === "blogs") {
      Object.assign(row, {
        authorEmail: user.email,
        publishedAt: (item.data as { status: string }).status === "Published" ? now : null,
      });
    }
    return Response.json({ item: await insertRow(tables[resource.data], row) }, { status: 201 });
  } catch (error) {
    return errorResponse(error, "Content could not be created.");
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const item = schemas[resource.data].safeParse(payload.item);
    if (!item.success || item.data.id !== id.data) {
      return Response.json({ error: item.success ? "The URL slug cannot be changed." : item.error.issues[0]?.message ?? "Invalid content" }, { status: 400 });
    }
    ...
    if (resource.data === "blogs") {
      Object.assign(row, {
        authorEmail: user.email,
        publishedAt: (item.data as { status: string }).status === "Published" ? new Date().toISOString() : null,
      });
    }
    const [updated] = await updateRows<Record<string, unknown>>(tables[resource.data], row, { id: id.data });
    return Response.json({ item: updated });
  } catch (error) {
    return errorResponse(error, "Content could not be updated.");
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return errorResponse(error, "Content could not be removed.");
  }
}
